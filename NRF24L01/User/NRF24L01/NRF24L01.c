...
/*配置MISO and MOSI SCLK 为复用功能（推挽）输出  Open205R_SPIx  */
...
u8 SPI_RW_Reg(u8 reg,u8 value)
{
	u8 status;
	CSN(0);
	status=SPI_Receive_byte(Open205R_SPIx,reg);   //select register  and write value to it
	SPI_Send_byte(Open205R_SPIx,value);   
	CSN(1);
	return(status); 
}
...
u8 SPI_Read_Reg(u8 reg)
{
	u8 status;
	CSN(0);
	SPI_Send_byte(Open205R_SPIx,reg);
	status=SPI_Receive_byte(Open205R_SPIx,0);   //select register  and write value to it
	CSN(1);
	return(status);
}
/********读出bytes字节的数据*************************/
u8 SPI_Read_Buf(u8 reg,u8 *pBuf,u8 bytes)
{
	u8 status,byte_ctr;
	CSN(0);
	status=SPI_Receive_byte(Open205R_SPIx,reg);       
	for(byte_ctr=0;byte_ctr<bytes;byte_ctr++)
		pBuf[byte_ctr]=SPI_Receive_byte(Open205R_SPIx,0);
	CSN(1);
	return(status);
}
...
u8 SPI_Write_Buf(u8 reg,u8 *pBuf,u8 bytes)
{
	...
	delay1us(1);
	for(byte_ctr=0;byte_ctr<bytes;byte_ctr++)
		SPI_Send_byte(Open205R_SPIx,*pBuf++);
	CSN(1);
	return(status);
}
 /****************发送函数***************************/
...
void nRF24L01_Initial(void)
{
	...
	RCC_AHB1PeriphClockCmd(Open205R_CSN_GPIO_CLK | Open205R_CE_GPIO_CLK | Open205R_IRQ_GPIO_CLK ,ENABLE);
	/*CE CSN Initial*/
	GPIO_InitStructure.GPIO_Pin =Open205R_GPIO_Pin_CE;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;
	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;
	GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;
	GPIO_Init(Open205R_CE_GPIO, &GPIO_InitStructure);	

 	GPIO_InitStructure.GPIO_Pin =Open205R_GPIO_Pin_CSN;
	GPIO_Init(Open205R_CSN_GPIO, &GPIO_InitStructure);

	/*IRQ CSN Initial*/
	GPIO_InitStructure.GPIO_Pin = Open205R_GPIO_Pin_IRQ;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN;
	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;
	GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;
	GPIO_Init(Open205R_IRQ_GPIO, &GPIO_InitStructure);

	Initial_SPI(Open205R_SPIx);
