    ...
    if(demo.select != prev_select)
    {
      ...
      if(demo.select & 0x80)
      {
        ...
        case 2:
          ...
          LCD_UsrLog("Core  : STM32F2xx.\n");  
          LCD_UsrLog("USB Host Library v2.0.0.\n");  
          LCD_UsrLog("USB Device Library v1.0.0.\n"); 
          LCD_UsrLog("USB OTG Driver v2.0.0\n");  
          LCD_UsrLog("STM32 Std Library v1.0.0.\n");          
#else

          LCD_UsrLog("Board : STM3210C-Eval.\n");  
          LCD_UsrLog("Core  : STM32F10x.\n");  
          LCD_UsrLog("USB Host Library v2.0.0.\n");  
          LCD_UsrLog("USB Device Library v1.0.0.\n"); 
          LCD_UsrLog("USB OTG Driver v2.0.0\n");            
          LCD_UsrLog("STM32 Std Library V3.5.0.\n");  
#endif          
        ...
        if(demo.select != prev_select)
        {
          ...
          if(demo.select & 0x80)
          {
            ...
            case 3:
              ...
              USB_OTG_Core.host.port_cb->DisconnHandled = 1; /* Handle to avoid the Re-entry*/
...
static void Show_Image(void)
{
  ...
  while (HCD_IsDeviceConnected(&USB_OTG_Core))
  {
    ...
    for(i = 0 ; i < IMAGE_BUFFER_SIZE; i+= 2)
    {
      ...
      LCD_WRITE_DATA(Image_Buf[i+1] << 8 | Image_Buf[i]);		
    } 
  }
	 LCD_RS_L();
	 LCD_CS_H();
