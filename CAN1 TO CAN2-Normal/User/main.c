...
int main(void)
{
  ...
  printf("Please press the UUSer middle button to continue!\r\n");

  Key_Config();
  CAN1_Config();
  CAN2_Config(); // RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOA, ENABLE);
  NVIC_Config();
    /* Infinite loop */
  while (1)
	{
	  while(GPIO_ReadInputDataBit(USER_KEY_Port,USER_KEY_Pin));
	  Can1WriteData(0x711); //标准ID 11位
	 if(Can2Flag== ENABLE)// while(Can2Flag!= ENABLE);
	 {Can2Flag = DISABLE;	 
	  ...
	  CAN2_DATA7!=CAN1_DATA7 )
	   {printf("CAN1 发送或者CAN2 接收有问题\r\n");error_can=1;}
		 else{printf("CAN1发送成功，CAN2接收成功\r\n");error_can=0;}
	 }
      Can2WriteData(0x101);
	  if(Can1Flag== ENABLE)//while(Can1Flag!= ENABLE);
    {  Can1Flag = DISABLE; 
...
void Can1WriteData(uint16_t ID)
{
  CanTxMsg TxMessage;
  u32 i = 0;
  CAN1_DATA0=0x00;///rand()%0xff;
  CAN1_DATA1=0x01;///rand()%0xff;  
  CAN1_DATA2=0x02;//rand()%0xff;
  CAN1_DATA3=0x03;//rand()%0xff;  
  CAN1_DATA4=0x04;//rand()%0xff;
  CAN1_DATA5=0x05;//rand()%0xff;
  CAN1_DATA6=0x06;//rand()%0xff;
  CAN1_DATA7=0x07;//rand()%0xff; 
...
// 	  while((CAN_TransmitStatus(CAN1,0) != CANTXOK))// && (i != 0x01)
//   {
//     i++;
//   }
}
